...
**[SecRandom](https://github.com/SECTL/SecRandom)** 是一款公平随机抽取系统，面向课堂、团队、活动、决策等场景，采用智能动态权重算法确保每位成员获得公平的抽取机会。SecRandom v3 基于 **C#/.NET 10** 开发，使用 **Avalonia + FluentAvalonia** 构建跨平台 UI，采用 Microsoft Fluent Design 设计语言，集**公平性、易用性、美观性**于一体。
...
#### 点名模式
- 支持按名单范围、分组、性别与人数抽取，贴合真实场景
- 支持普通随机、历史平衡及重复控制
- 单人/多人抽取，抽取动画丰富

#### 闪抽模式
- 通过独立悬浮窗快速抽取学生，无需切换窗口
- 支持快捷键触发与自定义按钮
